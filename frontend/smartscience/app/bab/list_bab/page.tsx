...
export default function ListBabPage() {
    ...
    useEffect(() => {
        ...
        if (paramKelas && paramMapel) {
            if (kelasList.length > 0 && mataPelajaranList.length > 0) {
                setFormData(prev => ({ ...prev, id_kelas: paramKelas, id_mapel: paramMapel }));
                handleFilter(undefined, { id_kelas: paramKelas, id_mapel: paramMapel });
            }
        }
    }, [searchParams]);
    ...
    const handleMataPelajaranChange = (value: string) => {
        router.push(`/bab/list_bab?id_kelas=${formData.id_kelas}&id_mapel=${value}`)
        const nama_mapel = mataPelajaranList.find((mp) => mp.id_mapel.toString() === value)?.nama_mapel || ""
        setFormData((prev) => ({ ...prev, id_mapel: value, nama_mapel: nama_mapel }))
    }

    const handleFilter = async (e?: React.FormEvent, filterIds?: { id_kelas: string, id_mapel: string }) => {
        if (e) e.preventDefault()
        if (!formData.id_kelas || !formData.id_mapel) {
            setAlertData({
                title: "Gagal",
        ...
        try {
            const payload = {
                id_mapel: Number(filterIds ? filterIds.id_mapel : formData.id_mapel),
            ...
            if (response.data && response.data.bab) {
                ...
                setAlertData({
                    title: "Gagal",
                    description: "Bab tidak ditemukan untuk mata pelajaran dan kelas ini.",
                    isSuccess: false
                })
                setAlertOpen(true)
            }
        ...
        <div className="flex min-h-screen bg-muted/40">
            ...
            <div className="flex flex-1 flex-col">
                <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background px-6 shadow-sm">
                    <h1 className="text-xl font-semibold md:text-2xl">List Bab</h1>
                    ...
                    <div className="mx-auto max-w-auto space-y-6">
                        ...
                        <Card className="border-t-4 border-t-primary shadow-md">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Pilih kelas dan mata pelajaran untuk melihat bab.
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                            ...
                            <CardContent>
                                <div className="rounded-md border">
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                <TableHead className="w-[100px]">ID Bab</TableHead>
                                            ...
                                            {isLoading ? (
                                                ...
                                                babList.map((item) => (
                                                    <TableRow key={item.id_bab}>
                                                        <TableCell className="font-medium">{item.id_bab}</TableCell>
                                                        <TableCell>{formData.nama_kelas || "-"}</TableCell>
                                                        <TableCell>{formData.nama_mapel || "-"}</TableCell>
                                                        <TableCell>{item.nama_bab}</TableCell>
                                                        <TableCell>
                                                            <Button variant="destructive" size="sm" onClick={() => handleDeleteClick(item.id_bab)}>
                    ...
                    <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Apakah Anda yakin ingin menghapus bab ini dari database? Tindakan ini tidak dapat dibatalkan.
                                </DialogDescription>
                            </DialogHeader>
                            <DialogFooter>
