...
import { useMemo } from "react"; // Tambahkan di baris import
...
export function TambahSoalContent() {
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        setCurrentUser(
          decoded.sub || decoded.username || decoded.role || "unknown",
        );
      } catch (error) {
  ...
  const handleKelasChange = async (value: string) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/v1/mapel/list_mapel`,
        payload,
      );
  ...
  const handleMapelChange = async (value: string) => {
    ...
    try {
      const payload = {
        id_mapel: parseInt(value),
        id_kelas: parseInt(selectedKelasId),
      };
      const response = await axios.post(
        `${API_URL}/api/v1/bab/list_bab`,
        payload,
      );
  ...
  const fetchSoalTOB = async (id: number) => {
    ...
    try {
      const response = await axios.post(
        `${API_URL}/api/v1/tob/post/list_soal_tob`,
        {
          id_tob: id,
        },
      );
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      console.log("[DEBUG] API Response Data:", response.data); // Wajib untuk audit struktur JSON

      // Ekstraksi data secara agresif (mengakomodasi standar Go/FastAPI)
      const rawData =
        response.data?.soal ||
        response.data?.data ||
        response.data?.items ||
      ...
      if (Array.isArray(rawData) && rawData.length > 0) {
        ...
        alert(
          response.data?.message ||
            "Data kosong: Tidak ada soal untuk bab ini di database.",
        );
      }
    } catch (error) {
      console.error("[ERROR] Fetch List Soal:", error);
      setListSoal([]);
  ...
  const filteredSoalList = useMemo(() => {
    if (!listSoal.length) return [];

    // Hash map untuk lookup O(1)
    const tobSoalIds = new Set(listSoalTOB.map((tob) => tob.id_soal));

    return listSoal.filter((item) => !tobSoalIds.has(item.id_soal));
  }, [listSoal, listSoalTOB]);
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Pilih kelas, mata pelajaran dan bab untuk melihat soal.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col sm:flex-row gap-4 items-end">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={selectedBabId}
                    onValueChange={setSelectedBabId}
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={isLoading || !selectedBabId}
                  >
                    {isLoading ? (
                    ...
                    <TableBody>
                      {isLoading ? (
                        ...
                        <TableRow>
                          ...
                          >
                            {selectedBabId
                              ? "Tidak ada soal tersisa untuk bab ini (semua sudah masuk TOB)."
                              : "Pilih bab dan cari."}
                          </TableCell>
                        </TableRow>
                      ) : (
                        filteredSoalList.map((item) => (
                          <TableRow key={item.id_soal}>
                            ...
                            <TableCell>
                              <Button
                                onClick={() => handlePlusClick(item.id_soal)}
                              >
          ...
          <Dialog open={plusDialogOpen} onOpenChange={setPlusDialogOpen}>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Apakah Anda yakin ingin tambah soal ini ke TOB? Tindakan ini
              ...
              <DialogFooter>
                <Button
                  variant="outline"
                  onClick={() => setPlusDialogOpen(false)}
                >
                  Batal
                </Button>
                <Button variant="destructive" onClick={confirmPlus}>
                  Tambah
                </Button>
              </DialogFooter>
            </DialogContent>
